...
const createProduct = (entry) => async (ctx) => {
  ...
  if (entry.hooks.api["beforeCreateProduct"]) {
    body =
      (await entry.hooks.api["beforeCreateProduct"](entry, ctx, body)) || body;
  }

  body.user = body.user || ctx.state?.user?.id;
  let resData = await entry.services["product"].create(
    body,
    ctx.state?.user?.id
  );
  ...
  if (entry.hooks.api["afterCreateProduct"]) {
    resData =
      (await entry.hooks.api["afterCreateProduct"](
        entry,
        ctx,
        resData,
        body
...
const updateProduct = (entry) => async (ctx) => {
  ...
  if (
    !(await userHaveRoles(ctx, ["admin"], entry.models.userRole)) &&
    !(await userIsOwner(
      ctx,
      body,
      entry.models.product,
  ...
  if (entry.hooks.api["beforeUpdateProduct"]) {
    body =
      (await entry.hooks.api["beforeUpdateProduct"](entry, ctx, body)) || body;
  }

  body.user = body.user || ctx.state?.user?.id;
  let resData = await entry.services["product"].update(
    body,
    ctx.state?.user?.id
  );
  ...
  if (entry.hooks.api["afterUpdateProduct"]) {
    resData =
      (await entry.hooks.api["afterUpdateProduct"](
        entry,
        ctx,
        resData,
        body
...
const removeProduct = (entry) => async (ctx) => {
  ...
  if (entry.hooks.api["beforeRemoveProduct"]) {
    body =
      (await entry.hooks.api["beforeRemoveProduct"](entry, ctx, body)) || body;
  }

  body.user = body.user || ctx.state?.user?.id;
  let resData = await entry.services["product"].remove(
    body.id,
    ctx.state?.user?.id
  );
  ...
  if (entry.hooks.api["afterRemoveProduct"]) {
    resData =
      (await entry.hooks.api["afterRemoveProduct"](
        entry,
        ctx,
        resData,
        body
...
const oneProduct = (entry) => async (ctx) => {
  ...
  if (entry.hooks.api["beforeOneProduct"]) {
    body =
      (await entry.hooks.api["beforeOneProduct"](entry, ctx, body)) || body;
  }

  body.user = body.user || ctx.state?.user?.id;
  let resData = await entry.services["product"].one(
    body.id,
    ctx.state?.user?.id
  );
  ...
  if (entry.hooks.api["afterOneProduct"]) {
    resData =
      (await entry.hooks.api["afterOneProduct"](entry, ctx, resData, body)) ||
      resData;
  }
...
const allProduct = (entry) => async (ctx) => {
  ...
  if (entry.hooks.api["beforeAllProduct"]) {
    body =
      (await entry.hooks.api["beforeAllProduct"](entry, ctx, body)) || body;
  }
  ...
  if (entry.hooks.api["afterAllProduct"]) {
    resData =
      (await entry.hooks.api["afterAllProduct"](entry, ctx, resData, body)) ||
      resData;
  }
...
const countProduct = (entry) => async (ctx) => {
  ...
  let resData = await entry.services["product"].count(
    query,
    ctx.state?.user?.id
  );
